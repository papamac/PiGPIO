...
FUNCTION:  rgpio.py is a library of Python classes and methods that interfaces
 ...
 VERSION:  0.2.2
    DATE:  May 3, 2023
  SOURCE:  This module was copied from https://github.com/joan2937/lg
...
class sbc():
    ...
    def group_claim_input(self, handle, gpio, lFlags=0):
        ...
        if len(gpio):
            ...
            return _u2i(_lg_command_ext(
                self.sl, _CMD_GSGIX, (len(gpio) + 2) * 4, [ext],
                L=len(gpio) + 2))
        else:
            return 0
    ...
    def tx_servo(self, handle, gpio, pulse_width,
        ...
        """
        ext = [struct.pack("IIIIII", handle & 0xffff, gpio,
                           pulse_width, servo_frequency, pulse_offset,
                           pulse_cycles)]
    ...
    def tx_wave(self, handle, gpio, pulses):
        ...
        if len(pulses):
            ...
            ext = [ext1, ext2]
            return _u2i(
                _lg_command_ext(self.sl, _CMD_GWAVE, size, ext, Q=q, L=l))
        else:
            return 0
    ...
    def i2c_write_block_data(self, handle, reg, data):
        ...
        return _u2i(
            _lg_command_ext(self.sl, _CMD_I2CWK, 8 + len(data), ext, L=2))
    ...
    def i2c_read_block_data(self, handle, reg):
        ...
        with self.sl.l:
            bytes = u2i(
                _lg_command_ext_nolock(self.sl, _CMD_I2CRK, 8, ext, L=2))
            if bytes > 0:
                rdata = self._rxbuf(bytes)
        return _u2i_list([bytes, rdata])
    ...
    def i2c_block_process_call(self, handle, reg, data):
        ...
        with self.sl.l:
            bytes = u2i(
                _lg_command_ext_nolock(self.sl, _CMD_I2CPK, 8 + len(data), ext,
                                       L=2))
            if bytes > 0:
                rdata = self._rxbuf(bytes)
        return _u2i_list([bytes, rdata])
    ...
    def i2c_write_i2c_block_data(self, handle, reg, data):
        ...
        ext = [struct.pack("II", handle, reg)] + [data]
        return _u2i(
            _lg_command_ext(self.sl, _CMD_I2CWI, 8 + len(data), ext, L=2))
    ...
    def i2c_read_i2c_block_data(self, handle, reg, count):
        ...
        with self.sl.l:
            bytes = u2i(
                _lg_command_ext_nolock(self.sl, _CMD_I2CRI, 12, ext, L=3))
            if bytes > 0:
                rdata = self._rxbuf(bytes)
        return _u2i_list([bytes, rdata])
    ...
    def i2c_write_device(self, handle, data):
        ...
        ext = [struct.pack("I", handle)] + [data]
        return _u2i(
            _lg_command_ext(self.sl, _CMD_I2CWD, 4 + len(data), ext, L=1))
    ...
    def script_run(self, handle, params=None):
        ...
        if params is not None:
            ...
            nump = 1 + len(params)
        return _u2i(
            _lg_command_ext(self.sl, _CMD_PROCR, nump * 4, [ext], L=nump))
    ...
    def script_update(self, handle, params=None):
        ...
        if params is not None:
            ...
            nump = 1 + len(params)
        return _u2i(
            _lg_command_ext(self.sl, _CMD_PROCU, nump * 4, [ext], L=nump))
    ...
    def serial_open(self, tty, baud, ser_flags=0):
        ...
        return _u2i(
            _lg_command_ext(self.sl, _CMD_SERO, 8 + len(tty), ext, L=2))
    ...
    def serial_write(self, handle, data):
        ...
        ext = [struct.pack("I", handle)] + [data]
        return _u2i(
            _lg_command_ext(self.sl, _CMD_SERW, 4 + len(data), ext, L=1))
    ...
    def shell(self, shellscr, pstring=""):
        ...
        return _u2i(
            _lg_command_ext(self.sl, _CMD_SHELL, 4 + ls + lp, ext, L=1))
    ...
    def spi_read(self, handle, count):
        ...
        with self.sl.l:
            bytes = u2i(
                _lg_command_ext_nolock(self.sl, _CMD_SPIR, 8, ext, L=2))
            if bytes > 0:
                rdata = self._rxbuf(bytes)
        return _u2i_list([bytes, rdata])
    ...
    def spi_write(self, handle, data):
        ...
        ext = [struct.pack("I", handle)] + [data]
        return _u2i(
            _lg_command_ext(self.sl, _CMD_SPIW, 4 + len(data), ext, L=1))
    ...
    def set_user(self, user="default",
        ...
        with self.sl.l:
            ...
            salt1 = "{:015x}".format(
                (int(time.time() * 1e7)) & 0xfffffffffffffff)
            ext = salt1 + '.' + user
            bytes = u2i(_lg_command_ext_nolock(
